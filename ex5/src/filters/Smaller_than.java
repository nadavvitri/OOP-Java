...
/**
 * This class extend Filter class and represent the "smaller_than" filter - returns files that there sizes
 * are strictly less than the given numbers of k-bytes.
 */
public class Smaller_than extends Filter {
    ...
    /**
     * Constructor
     * @param biggerSize - the upper size bound.
     ...
     * "YES" or "NO".
     */
    public Smaller_than(double biggerSize, boolean isNot, int lineNum) throws Type1Exception {
        if (lineNum >= 0){
    ...
    public boolean accept(File pathname) {
        if (pathname.isFile()) {
            if (not) {
                return pathname.length() / KILO > biggerSize;
            } else {
                return pathname.length() / KILO < biggerSize;
            }
        }return false;
